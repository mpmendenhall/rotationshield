...
void vis_test_sequence(ReactiveSet* RS, MagExtField* MxS, vec3 vs = vec3(0.5,-0.5,0.5), vec3 vs0 = vec3(0,0,0)) {
    ...
    MxS->visualize();
    cout << "B non-interacting: " << std::endl;
    qsurvey(MxS,vs,vs0);
    vsr::pause();
...
void superball_test(unsigned int ngrid) {
    ...
    MagExtField* MxS = new MagExtField();
    vec3 B0(1,1,1);
    UniformField* BU = new UniformField(B0);
    MxS->addsource(BU);
    cout << "Expelling external B = " << B0 << " field from a superconducting sphere.\n";
...
void vectors_test() {
    ...
    vec2 sx0(0.5,0.5);
    vec3 x0 = (*SG)(sx0);
    vec3 dx = SG->deriv(sx0,0);
    vec3 dy = SG->deriv(sx0,1);
    vec3 dxn = SG->deriv(sx0,0,true);
    vec3 dyn = SG->deriv(sx0,1,true);
    vec3 dz = SG->snorm(sx0,false); //cross(dx,dy);
    vec3 sn = SG->snorm(sx0,true);
    cout << "Surface position " << sx0 << " is space position " << x0 << "\n";
    cout << "Local coordinate vectors:\n";
    cout << "\tdx: " << dx << "\tnormalized: " << dxn << "\n";
    cout << "\tdy: " << dy << "\tnormalized: " << dyn << "\n";
    cout << "\tdz: " << dz << "\tnormalized: " << sn << "\n";
...
void mirror_test() {
    ...
    cout << "Mirroring cos theta coil current between superconducting plates for improved uniformity.\n";
...
void tube_test() {
    ...
    cout << "Cos theta coil in a ferromagnetic shield.\n";
...
void flux_trap_test() {
    ...
    cout << "Calculating trapped-flux state of superconducting ring..." << std::endl;
    ...
    cout << "Setting initial current loop distribution..." << std::endl;
    RS->setZeroState();
...
void two_torus_equilibration() {
    ...
    cout << "Initial field RMS inside 1: " << MxI->field_RMS_near(*RS1->mySurface,-0.1) << "\n";
    cout << "Initial field RMS inside 2: " << MxI->field_RMS_near(*RS2->mySurface,-0.1) << "\n";
    ...
    //
    MRC->visualize();
    //cout << "Non-interacting field RMS inside 1: " << Ftot->field_RMS_near(*RS1->mySurface,-0.1) << "\n";
    //cout << "Non-interacting field RMS inside 2: " << Ftot->field_RMS_near(*RS2->mySurface,-0.1) << "\n";
    ...
    cout << "Interacting field RMS inside 1: " << Ftot->field_RMS_near(*RS1->mySurface,-0.1) << "\n";
    cout << "Interacting field RMS inside 2: " << Ftot->field_RMS_near(*RS2->mySurface,-0.1) << "\n";
    
    cout << "\nIterative method calculation complete in " << difftime(end_time,start_time) << " seconds.\n";
    cout << "Calculating brute-force solution...\n\n";
    ...
    cout << "Final state difference = " << (final_iterative - MRC->finalState).mag()/MRC->finalState.mag() << "%\n";
