...
void NameSelector::addSynonym(string arg0, string syn) {
    std::map<string,unsigned int>::iterator it = nameMap.find(arg0);
...
void NameSelector::doIt() {
    ...
    do {
        ...
        while(1) {
            ...
            std::map<string,unsigned int>::iterator it = nameMap.find(myArg);
            
            // if no direct match, apply soft matching when available
            if(it == nameMap.end() && softmatch) {
                
                // find soft matches
                vector<std::map<string,unsigned int>::iterator> matches;
                for(std::map<string,unsigned int>::iterator i = nameMap.begin(); i != nameMap.end(); i++)
